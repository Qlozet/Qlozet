...
import { Edit3, Plus, Search, Shirt, Trash2, Shield } from 'lucide-react';
import {
  AddStylesModal,
  type CreatedStyle,
  type EditStyleSeed,
} from './add-styles-modal';
...
export const ManageStylesModal = NiceModal.create(() => {
  ...
  // Editing reuses the SAME modal as adding (like fabric/accessory editing) -
  // full form incl. category, audience, tags and image, instead of the old
  // 3-field inline card edit.
  const startEdit = async (style: (typeof styles)[0]) => {
    await NiceModal.show(AddStylesModal, {
      editStyle: {
        id: style.id,
        name: style.name,
        ...
        priceSuggestion: style.priceSuggestion,
      } satisfies EditStyleSeed,
    });
  };
