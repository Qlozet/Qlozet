...
export const HotspotEditorModal = NiceModal.create(
  ({ imageUrl, hotspots: initialHotspots, sections }: HotspotEditorProps) => {
    ...
    const availableSections = sections.flatMap((sec) => {
      if (sec.key === 'style') {
        ...
        sec.items?.forEach((item) => {
          ...
          const cat =
            item.category ||
            item.originalData?.categories?.[0] ||
    ...
    const activeHotspot =
      activeHotspotIndex !== null ? hotspots[activeHotspotIndex] : null;
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {!activeHotspot ? (
                <div className="flex h-full flex-col items-center justify-center text-center text-muted-foreground opacity-70">
                  <p className="text-sm">
                    Select or place a hotspot on the image to edit its
                    properties.
                  </p>
                </div>
              ) : (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateActiveHotspot({ label: e.target.value })
                      }
                      placeholder="e.g. Sleeves, Collar"
                    />
                    <p className="text-[10px] text-muted-foreground">
                      Internal name to identify this hotspot.
                    </p>
                  </div>

                  <div className="space-y-2">
                    <label className="text-sm font-medium text-foreground">
                      Target Section (Field Key)
                    </label>
                    {availableSections.length > 0 ? (
                      <Select
                        value={activeHotspot.field_key}
                        onValueChange={(val) =>
                          updateActiveHotspot({ field_key: val })
                        }
                      >
                      ...
                      <div className="text-sm text-destructive bg-destructive/10 p-3 rounded-md">
                        No other customization sections available to target. Add
                        more sections to your product first.
                      </div>
                    )}
                    <p className="text-[10px] text-muted-foreground">
                      The customization section this hotspot links to.
                    </p>
                  </div>
