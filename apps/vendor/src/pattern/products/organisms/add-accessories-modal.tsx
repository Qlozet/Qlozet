...
import {
  AUDIENCE_OPTIONS,
  type Audience,
} from './clothing-organization-section';
...
export const AddAccessoryModal = create(({ editId }: { editId?: string }) => {
  ...
  const [createAccessory, { isLoading: isCreating }] =
    useCreateAccessoryMutation();
  ...
  useEffect(() => {
    if (productData) {
      ...
      if (pImages && pImages.length > 0) {
        const imgObj =
          typeof pImages[0] === 'string'
            ? { url: pImages[0], public_id: 'unknown' }
            : pImages[0];
  ...
  // Taxonomy hooks
  const { data: accessoryTypes, isLoading: isLoadingTypes } =
    useGetProductTypesQuery('accessory');
  // Skip categories query if product_type isn't a real taxonomy type (e.g. generic 'accessory')
  const isValidProductType = Boolean(
  ...
  const handleAddVariants = () => {
    ...
    const existingColorValues = variants
      ?.map((v) => {
        const colorOption = allColors.find((c) => c.hex === v.colorHex);
        return colorOption ? colorOption.value : null;
      })
      .filter(Boolean);
    const newColorValues = selectedColors.filter(
      (colorValue) => !existingColorValues.includes(colorValue)
    );
    ...
    const newVariants: VariantRow[] = newColorValues?.map((colorValue) => {
      ...
      const availableSizes =
        selectedSizes?.map(
          (sizeValue) => sizeMap[sizeValue] || sizeValue.toUpperCase()
        ) || [];
    ...
    try {
      ...
      const variantsToSubmit = await Promise.all(
        variants.flatMap(async (v) => {
          ...
          return Promise.all(
            v.availableSizes.map(async (size) => {
      ...
      await createAccessory({
        ...
        accessory: {
          ...
          taxonomy: {
            ...
            attributes: categoryData?.attributes?.length
          ...
          images: finalImageUrl
            ? [{ url: finalImageUrl, public_id: finalPublicId }]
            : existingImage
              ? [existingImage]
              : [],
        },
      ...
      toast.error(
        editId ? 'Failed to update accessory.' : 'Failed to create accessory.'
      );
    }
  };
          ...
          <div className="bg-[hsla(0,0%,96%,1)] dark:bg-[#4A4949] space-y-3 p-4 sm:p-7 lg:overflow-y-auto h-full">
            ...
            {isLoadingProduct ? (
              ...
              <>
                ...
                <div
                  onClick={() =>
                    document.getElementById('accessory-image')?.click()
                  }
