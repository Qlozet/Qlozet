...
/** Seed for edit mode - pass as NiceModal.show(AddStylesModal, { editStyle }). */
export interface EditStyleSeed {
  id: string;
  name: string;
  description?: string;
  /** Backend gender value: male | female | unisex. */
  gender?: string;
  /** Backend category value, e.g. 'full_body'. */
  category?: string;
  tags?: string[];
  imageUrl?: string;
...
/** 'full_body' -> 'Full Body' (the labels STYLE_TYPES uses). */
const categoryToLabel = (c?: string): string =>
  c
    ? c
        .split('_')
...
export const AddStylesModal = NiceModal.create<{ editStyle?: EditStyleSeed }>(
  ({ editStyle }) => {
    ...
    const [audience, setAudience] = useState<'men' | 'women'>(
      editStyle?.gender === 'female' ? 'women' : 'men'
    );
    const [categories, setCategories] = useState<string[]>([]);
    const [styleType, setStyleType] = useState(
      categoryToLabel(editStyle?.category)
    );
    const [tags, setTags] = useState<string[]>(editStyle?.tags ?? []);
    const [price, setPrice] = useState(
      editStyle?.priceSuggestion && editStyle.priceSuggestion > 0
        ? String(editStyle.priceSuggestion)
        : ''
    );
    const [imageUrl, setImageUrl] = useState<string | null>(
      editStyle?.imageUrl ?? null
    );
    const [showUrlInput, setShowUrlInput] = useState(false);
    const [urlValue, setUrlValue] = useState('');

    const [createVendorStyle, { isLoading: isCreating }] =
      useCreateVendorStyleMutation();
    const [updateVendorStyle, { isLoading: isSavingEdit }] =
      useUpdateVendorStyleMutation();
    const [generateStyleImage, { isLoading: isGenerating }] =
      useGenerateStyleImageMutation();
    const [uploadProductImage, { isLoading: isUploadingImg }] =
    ...
    const handleFile = async (file?: File) => {
      ...
      try {
        const res: any = await uploadProductImage(file).unwrap();
        const url = res?.data?.url ?? res?.url ?? res?.data?.imageUrl;
        if (!url) throw new Error('no url');
        setImageUrl(url);
      } catch (err: any) {
        toast.error(readApiError(err, 'Image upload failed'));
      }
    };
    ...
    const upload = async () => {
      ...
      try {
        ...
        else if (['dresses', 'outwear', 'traditional', 'kaftan'].includes(cat))
          mappedType = 'full_body';

        const shared = {
          name: name.trim(),
          category: styleType.toLowerCase().replace(' ', '_'),
          description: description.trim() || undefined,
          gender: (audience === 'men' ? 'male' : 'female') as 'male' | 'female',
          image_url: imageUrl ?? undefined,
          attributes: tags,
          ...(price ? { price_suggestion: parseFloat(price) } : {}),
        };

        // Same modal, two verbs: update when editing, create otherwise.
        const res = editStyle
          ? await updateVendorStyle({ id: editStyle.id, data: shared }).unwrap()
          : await createVendorStyle({
              ...shared,
        ...
        modal.resolve({
          id: (res as any)._id ?? editStyle?.id,
          name: (res as any).name ?? shared.name,
          styleCode: (res as any).style_code,
          audience,
          categories,
          styleType: (res as any).category ?? shared.category,
          tags: (res as any).attributes || tags,
          imageUrl: (res as any).image_url ?? imageUrl ?? undefined,
        } satisfies CreatedStyle);
        modal.remove();
        toast.success(
          editStyle
            ? 'Style updated'
            : 'Style successfully added to your library!'
        );
      } catch (err: any) {
        toast.error(
          readApiError(
            err,
            editStyle ? 'Failed to update style.' : 'Failed to upload style.'
          )
        );
      }
    };
        ...
        <DialogContent className="sm:max-w-4xl p-0 overflow-hidden sm:rounded-[16px]">
          <div className="grid max-h-[90vh] w-full grid-cols-1 overflow-y-auto sm:rounded-[16px] md:grid-cols-2 md:overflow-hidden">
            ...
            <div className="space-y-4 p-4 sm:p-6 bg-card md:overflow-y-auto custom-scrollbar">
              ...
              <div>
                <FieldLabel htmlFor="style-description">
                  Style description
                </FieldLabel>
                <textarea
              ...
              <div>
                <FieldLabel
                  htmlFor="style-price"
                  tooltip="Suggested add-on price when this style is applied"
                >
                  Suggested price (NGN)
                </FieldLabel>
                <Input
              ...
              <button
                type="button"
                onClick={upload}
                disabled={isCreating || isSavingEdit || isUploadingImg}
                ...
                <div className="flex min-h-[280px] flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-input p-6 text-center">
                  ...
                  >
                    {isGenerating ? (
                      ...
                      <Sparkles className="size-4 text-[#2F80ED]" />
                    )}
                    Generate with AI
                  </button>
