...
// fabric.product_type (required on the schema) and its pattern at
// fabric.pattern — the previous hardcoded Natural/Synthetic families
// matched neither, so the tabs filtered nothing.
const ALL = 'All';

const fabricTypeOf = (f: FabricLike): string =>
  String(f.fabric?.product_type ?? f.product_type ?? f.material ?? '').trim();

const fabricPatternOf = (f: FabricLike): string =>
...
export const SelectFabricModal = NiceModal.create(() => {
  ...
  // Pattern sub-tabs for the chosen material (taxonomy `categories`).
  const subTabs =
    type === ALL
      ? []
      : [
