...
export function VariantSetVariants({
  variants,
  onVariantUpdate,
  onVariantDelete,
  onVariantToggle,
}: VariantSetVariantsProps) {
  return (
    <div className="bg-card rounded-lg p-4 md:p-6 border border-border mt-6 overflow-hidden flex flex-col">
      ...
      {variants?.length === 0 ? (
        <div className="text-center py-8 text-muted-foreground">
          <p className="text-sm">
            No variants created yet. Select colors and sizes above, then click
            "Add Variants".
          </p>
        </div>
      ) : (
...
function VariantRowComponent({
  ...
  return (
    <div className="border border-border rounded-lg overflow-hidden">
      {/* Collapsed Row */}
      <div
        className="flex gap-4 p-4 items-center bg-accent cursor-pointer hover:bg-accent/80"
        onClick={onToggle}
      >
        ...
        <div className="w-[25%] flex gap-2">
          {variant.sizes.slice(0, 5)?.map((_, idx) => (
            <div
              key={idx}
              className="w-8 h-8 rounded bg-orange-100 flex items-center justify-center"
            >
            ...
            >
              ...
              <div className="w-[28%]">
                <Input
                  ...
                  onChange={(e) =>
                    handleSizeUpdate(sizeIndex, 'sku', e.target.value)
                  }
