...
const dispute = (patch: Record<string, unknown> = {}) =>
  patch as unknown as Dispute;

describe('dispute field readers', () => {
  it('prefers the order reference, then the order id, then the dispute id', () => {
    expect(
      readDisputeRef(dispute({ order_reference: 'QLZ-1', _id: 'd1' }))
    ).toBe('QLZ-1');
  ...
  it('reads the reason, falling back to the description', () => {
    expect(readDisputeReason(dispute({ reason: 'Wrong size' }))).toBe(
      'Wrong size'
    );
  ...
  it('reads the customer handle, falling back to the email', () => {
    expect(
      readDisputeCustomer(dispute({ customer: { username: 'ada' } }))
    ).toBe('ada');
    expect(
      readDisputeCustomer(dispute({ customer: { email: 'a@b.co' } }))
    ).toBe('a@b.co');
    expect(readDisputeCustomer(dispute({}))).toBe('—');
  });

  it('lower-cases the status and defaults to open', () => {
    expect(readDisputeStatus(dispute({ status: 'UNDER_REVIEW' }))).toBe(
      'under_review'
    );
...
describe('disputeStatusBadge', () => {
  ...
  it('humanises an unknown status instead of rendering the raw key', () => {
    expect(disputeStatusBadge('awaiting_customer').label).toBe(
      'Awaiting customer'
    );
