...
export const OrderQuoteDrawer = create<OrderQuoteDrawerProps>(({ order }) => {
  ...
  return (
    <Sheet open={visible} onOpenChange={handleClose}>
      <SheetContent
        side='right'
        className='flex w-full flex-col gap-0 overflow-y-auto p-0 sm:max-w-md !top-6 !bottom-6 !right-6 !h-[calc(100vh-3rem)] rounded-2xl custom-card-shadow'
      >
        {/* Header */}
        <div className='flex items-start justify-between px-6 pb-3 pt-6'>
          <button
            ...
            onClick={handleClose}
            aria-label='Back'
            className='flex size-8 items-center justify-center rounded-full border border-border text-grey3 hover:bg-gray-50'
          >
        ...
        <div className='flex items-center justify-between px-6'>
          <div>
            <h2 className='text-lg font-bold text-grey-black'>
              Order #{readOrderId(order)}
            </h2>
            <p className='text-xs text-grey2'>
              {formatLongDate(order.createdAt ?? order.date)}
            </p>
          </div>
          <span className='inline-flex h-[26px] items-center rounded-[8px] bg-[#FEF6E7] px-3 text-xs font-medium text-[#DD900D]'>
            Pending
          </span>
        </div>
        ...
        <div className='space-y-5 px-6 py-5'>
          {/* Quote card */}
          <section className='space-y-4 rounded-xl bg-[#F8F9FA] p-4'>
            <div className='flex items-center justify-between'>
              <h3 className='text-base font-semibold text-grey-black'>Quote</h3>
            ...
            <div className='rounded-xl border border-border bg-white p-4'>
              <div className='mb-3 flex items-center justify-between'>
                <span className='text-sm font-semibold text-grey-black'>
                  Line Items
                </span>
                <Calculator className='size-4 text-grey3' />
              </div>
              ...
              <div className='space-y-3'>
                {lineItems.map((li, index) => {
                  ...
                  return (
                    ...
                    >
                      <span className='text-sm text-grey3'>{li.label}</span>
                      <div
                        className={cn(
                      ...
                      >
                        <span className='text-sm text-grey2'>$</span>
                        <input
                          type='number'
                          min={0}
                          value={li.amount}
                          ...
                          className={cn(
                            'w-full bg-transparent text-right text-sm text-grey-black focus:outline-none [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none',
                            readOnly && 'text-grey2'
                          )}
                        />
                      </div>
              ...
              <div className='mt-3 flex items-center justify-between border-t border-border pt-3'>
                ...
                <span className='text-sm font-bold text-grey-black'>
            ...
            <div className='flex items-center justify-between rounded-xl border border-border bg-white px-4 py-3'>
              ...
              <div className='flex items-center gap-2'>
                <input
                  ...
                  className='w-16 rounded-lg border border-border px-3 py-1.5 text-right text-sm text-grey-black focus:outline-none [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none'
                />
                <span className='text-sm text-grey3'>yards</span>
              </div>
            </div>
            ...
            <div className='flex items-center justify-between rounded-xl border border-border bg-white px-4 py-3'>
              <div>
                ...
                <p className='text-xs text-grey2'>Days from acceptance</p>
              </div>
              <div className='flex items-center gap-2'>
                <input
                  type='number'
                  ...
                  className='w-20 rounded-lg border border-border px-3 py-1.5 text-right text-sm text-grey-black focus:outline-none [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none'
                />
                <span className='text-sm text-grey3'>days</span>
              </div>
            </div>

            {/* Notes */}
            <div className='space-y-1.5'>
              <label className='text-xs font-medium uppercase tracking-wide text-grey2'>
                Notes to customer
              </label>
              <Textarea
                value={notes}
            ...
            <div className='flex items-start gap-2 rounded-lg bg-[#F1F1F1] p-3'>
              <Info className='mt-0.5 size-4 shrink-0 text-grey3' />
          ...
          <section className='space-y-3'>
            ...
            <div className='flex items-center gap-3 rounded-xl bg-[#F8F9FA] p-4'>
              ...
              <div>
                ...
                <div className='mt-1 flex flex-wrap gap-2'>
                  {SAMPLE_GARMENT_SPEC.tags.map((tag) => (
                    <span
                      key={tag}
                      className='rounded-md bg-white px-2 py-0.5 text-[11px] font-medium text-grey3'
                    >
                      {tag}
                    </span>
                  ))}
