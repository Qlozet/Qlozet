...
const SpecRow: React.FC<{ label: string; value: string; swatch?: string | null }> = ({
  label,
  value,
  swatch,
}) => (
    ...
    <div className='space-y-5'>
      ...
      <div className='flex items-start gap-3.5'>
        <div className='relative flex size-20 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-gray-100 dark:bg-gray-700'>
          {heroUrl ? (
            <Image src={heroUrl} alt={design?.name ?? 'Design'} fill className='object-cover' sizes='80px' />
          ) : (
      ...
      {/* Design images */}
      {images.length > 0 && (
        <Section title='Design Images' icon={<ImageIcon className='size-3.5 text-grey3' />}>
          <div className='flex gap-2 overflow-x-auto'>
            {images.map((img, i) => (
              <div key={i} className='relative size-24 shrink-0 overflow-hidden rounded-xl border border-[#E5E7EB] dark:border-border bg-gray-100'>
                <Image src={img} alt={`Design ${i + 1}`} fill className='object-cover' sizes='96px' />
              </div>
            ))}
          </div>
        </Section>
      ...
      {/* Fabric */}
      {fabricName && (
        <Section title='Fabric' icon={<Scissors className='size-3.5 text-grey3' />}>
          <div className='flex items-center gap-3 rounded-xl border border-[#E5E7EB] dark:border-border bg-[hsla(0,0%,96%,1)] dark:bg-[#4A4949] px-3 py-2.5'>
            <div
              ...
              style={!fabricImg && fabricSwatch ? { backgroundColor: fabricSwatch } : undefined}
            >
              {fabricImg ? (
                <Image src={fabricImg} alt={fabricName} fill className='object-cover' sizes='44px' />
              ) : !fabricSwatch ? (
                <Scissors className='size-4 text-gray-400' />
              ) : null}
            </div>
            <div className='min-w-0 flex-1'>
              <p className='truncate text-sm font-medium text-[#333333] dark:text-white'>{fabricName}</p>
              {pricePerYard ? (
      ...
      {/* Reference images */}
      {refImages.length > 0 && (
        <Section title='Reference Images' icon={<ImageIcon className='size-3.5 text-grey3' />}>
          <div className='flex gap-2 overflow-x-auto'>
            {refImages.map((img, i) => (
              <div key={i} className='relative size-20 shrink-0 overflow-hidden rounded-xl border border-[#E5E7EB] dark:border-border bg-gray-100'>
                <Image src={img} alt={`Reference ${i + 1}`} fill className='object-cover' sizes='80px' />
              </div>
            ))}
          </div>
        </Section>
...
export const DesignDetailModal = create<{ design: any }>(({ design }) => {
  ...
  return (
    <Dialog open={visible} onOpenChange={onOpenChange}>
      <DialogContent
        className={cn('sm:max-w-[520px] p-0 gap-0 bg-white dark:bg-card', 'z-[60]')}
      >
