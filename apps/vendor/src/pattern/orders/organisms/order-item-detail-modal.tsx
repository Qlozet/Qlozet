  ...
  return (
    <div className='space-y-5'>
      ...
      {!!item.color_variant_selections?.length && (
        <Section title='Colour & Size' icon={<Palette className='size-3.5 text-grey3' />}>
          {item.color_variant_selections.map((v, i) => {
            const cv = byId<ClothingColorVariantDoc>(clothing?.color_variants, v.variant_id);
            const swatch = cv?.hex ?? cv?.hex_code;
            const colourName = v.color ?? cv?.color_name ?? cv?.name ?? 'Standard';
            return (
              <SelectionRow
                key={i}
      ...
      {!!item.style_selections?.length && (
        <Section title='Styles' icon={<Layers className='size-3.5 text-grey3' />}>
          ...
          {item.fabric_selections.map((f, i) => {
            ...
            return (
              <SelectionRow
                ...
                subtitle={`${yards ?? '—'} yd${
                  fab?.price_per_yard ? ` · ${formatNaira(fab.price_per_yard)}/yd` : ''
                }`}
      ...
      {!!item.accessory_selections?.length && (
        <Section title='Accessories' icon={<Gem className='size-3.5 text-grey3' />}>
          {item.accessory_selections.map((a, i) => {
            const acc = byId<ClothingAccessoryDoc>(clothing?.accessories, a.accessory_id);
      ...
      {!!item.addon_selections?.length && (
        <Section title='Add-ons' icon={<PlusCircle className='size-3.5 text-grey3' />}>
          {item.addon_selections.map((ad, i) => {
            ...
            const variant = byId(addon?.variants as any, ad.variant_id) as
              | { name?: string; price?: number; color_hex?: string; image_url?: string }
              | undefined;
            return (
              <SelectionRow
                key={i}
