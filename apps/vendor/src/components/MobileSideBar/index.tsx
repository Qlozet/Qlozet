...
const MobileSideBar: React.FC<MobileSideBarProps> = ({
  ...
  useEffect(() => {
    const activeIndex = sidebaritems.findIndex(
      (item) => item.subItems && isItemActive(item.link)
    );
    if (activeIndex !== -1) {
  ...
  return (
    <div
      className={cn(
        ...
        showMobileNav
          ? 'opacity-100 visible block lg:hidden'
            ...
            if (item.subItems) {
              return (
                <Accordion
                  ...
                  className="w-full"
                  value={
                    expandedItem === `item-${index}` ? `item-${index}` : ''
                  }
                    ...
                    >
                      <div className="flex items-center gap-4">
                        {isActive ? (
                          <Image
                            src={item.activeIcon}
                            alt={`${item.name} active icon`}
                            className="size-5 dark:brightness-0 dark:invert transition-all"
                          />
                        ) : (
                          <Image
