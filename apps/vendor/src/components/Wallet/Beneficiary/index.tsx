...
import { type Beneficiary, useGetBeneficiariesQuery } from '@/redux/services/wallet/wallet.api-slice';
...
const Beneficiary: FC<BeneficiaryProps> = ({ closeModal }) => {
  const { data: beneficiariesData, isLoading } = useGetBeneficiariesQuery();
  const [beneficairy, setBeneficairy] = useState<Beneficiary[]>([]);
  const [searchValue, setSearchValue] = useState<string>('');

  useEffect(() => {
    if (beneficiariesData?.data) {
      const savedBeneneficiaries = beneficiariesData.data?.map((item) => ({
        id: item?.id || '',
        name: item?.name,
        ...
        schedulePayment: item?.schedulePayment || false,
