export const AUTH_ROUTES = {
  accountVerification: '/account-verification',
  signIn: '/auth/sign-in',
  register: '/register',
  signup: '/auth/sign-up',
  forgotPassword: '/auth/forgot-password',
  passwordResetCodeSent: '/auth/forgot-password/code-sent',
  resetPassword: '/auth/reset-password',
  createNewPassword: '/auth/create-new-password',
  verification: '/auth/verification',
  verifyAccountMessage: '/verify-account-message',
  awaitingVerification: '/auth/sign-up/awaiting-verification',
} as const;

export const APP_ROUTES = {
  home: '/',
  customers: '/customers',
  dashboard: '/dashboard',
  orders: '/orders',
  products: '/products',
  productsCloth: '/products/clothing',
  productsAccessories: '/products/accessories',
  productsFabrics: '/products/fabrics',
  productsCollections: '/products/collections',
  productsCollectionsCreate: '/products/collections/create',
  productsDiscounts: '/products/discounts',
  productsDiscountsCreate: '/products/discounts/create',
  productsDiscountsEdit: '/products/discounts/edit',
  productsSizeGuides: '/products/size-guides',
  productsSizeGuidesCreate: '/products/size-guides/create',
  productsSizeGuidesEdit: '/products/size-guides/edit',
  productsVideos: '/products/videos',
  productDetails: '/product-details',
  wallet: '/wallet',
  users: '/users',
  settings: '/settings',
  support: '/support',
  add: '/add',
  details: '/details',
  cookiePolicy: '/cookie-policy',
  privacyPolicy: '/privacy-policy',
  termsOfService: '/terms-of-service',
} as const;
