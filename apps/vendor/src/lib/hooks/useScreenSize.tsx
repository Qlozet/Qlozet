...
const getBreakpoint = (width: number): keyof typeof breakpoints => {
  return (
    (Object.keys(breakpoints) as (keyof typeof breakpoints)[]).find(
      (bp) => width <= breakpoints[bp]
    ) ?? '2xl'
  );
};
...
export const useScreenSize = () => {
  ...
  const lt = (bp: keyof typeof breakpoints) =>
    breakpoints[screenSize] < breakpoints[bp];
  const gt = (bp: keyof typeof breakpoints) =>
    breakpoints[screenSize] > breakpoints[bp];
