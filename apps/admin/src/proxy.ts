...
/**
 * Auth guard — Next.js 16 Proxy (formerly Middleware). Mirrors the vendor app's
 * guard so both consoles behave the same way.
 *
...
export const config = {
  ...
  // The dot rule is what keeps the metadata routes public: robots.txt,
  // manifest.webmanifest, icon.svg, apple-icon.png, opengraph-image.png and
  // favicon.ico all carry an extension, so crawlers and link unfurlers can
  // still fetch them without a session.
