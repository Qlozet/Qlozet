      ...
      <div className="bg-white  w-full lg:w-[775px] rounded-[16px]">
        ...
        <div className="flex items-center justify-start px-4 mt-6">
          <div className="bg-[#F6F6F6]  rounded-xl flex items-center font-medium text-[14px]">
            <button
              className={`${gender === "Male" ? "bg-primary text-white" : "text-[#495057]"
            ...
            <button
              ...
              className={`${gender === "Female" ? "bg-primary text-white" : "text-[#495057]"
        ...
        <div className="my-6 px-6">
          <div className="flex items-center mx-6">
            {styles.map((item, index) => (
              <span
                key={index}
                className={`h-[40px] px-3 flex items-center justify-center text-sm font-medium cursor-pointer rounded-t-[10px] ${currentNav === item ? "bg-[#F8F8F8F8]" : "text-[#495057]"
                  }`}
            ...
            <div className="bg-[#F8F8F8F1]">
              <div className="flex items-center mt-1 ">
                {stylesBottom.map((item, index) => (
                  <span
                    key={index}
                    className={`h-[40px] px-3 flex items-center justify-center text-sm font-medium cursor-pointer rounded-t-[10px] ${currentNav === item ? "bg-[#ECEBEB]" : ""
                      }`}
                    onClick={() => setCurrentNav(item)}
                  >
                    {item}
              ...
              <div className=" bg-[#ECEBEB] p-6 rounded-bl-[10px] rounded-br-[10px]">
                <div className="grid grid-cols-7 gap-x-2 gap-y-4 ">
                  {styleData.map((item) => (
                    <div
                      className={` ${formik.values.styles.filter(
