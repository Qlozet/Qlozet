...
import { completeProductSchema, CompleteProductData, createDefaultProductData, transformFormDataToApiData } from "@/lib/validations/product";
import { 
...
export const AddProductForm: React.FC<AddProductFormProps> = ({ 
  ...
  const onSubmit = async (data: CompleteProductData) => {
    try {
      ...
      if (!transformedData.name || !transformedData.category || !transformedData.price || transformedData.images.length === 0) {
            ...
            <div className="text-sm text-gray-600">
              ...
              {hasUnsavedImages && (
                <Typography variant="caption" className="text-amber-600 block">
                  You have {selectedFiles.length} unsaved image(s). They will be uploaded when you submit.
                </Typography>
              )}
            </div>
