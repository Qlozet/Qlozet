...
export const ProductDetailsCard: React.FC<ProductDetailsCardProps> = ({ 
  ...
  const totalStock = product.variants?.reduce((sum, variant) => sum + variant.stock, 0) || product.stock;
          ...
          <div>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <p className="mt-1 font-medium">
                  {customizationCount > 0 ? `${customizationCount} options` : 'No customizations'}
                </p>
              </div>
        ...
        {product.variants && product.variants.length > 0 && (
          <div>
            <div className="flex items-center space-x-2 mb-3">
              ...
              <h3 className="font-medium">Variants ({product.variants.length})</h3>
            </div>
              ...
              {product.variants.slice(0, 4).map((variant, index) => (
                <div key={variant._id} className="p-3 bg-gray-50 rounded-lg">
                  <div className="space-y-1 text-sm">
                    ...
                    {variant.material && (
                      <div><strong>Material:</strong> {variant.material}</div>
                    )}
                    <div><strong>Stock:</strong> {variant.stock}</div>
                    {variant.additionalPrice > 0 && (
                      <div><strong>Additional Price:</strong> +<ProductPrice price={variant.additionalPrice} showCurrency={false} /></div>
                    )}
                  </div>
                </div>
              ))}
        ...
        {product.customizations && product.customizations.length > 0 && (
          <div>
            <div className="flex items-center space-x-2 mb-3">
              ...
              <h3 className="font-medium">Customizations ({product.customizations.length})</h3>
            </div>
            <div className="space-y-3">
              {product.customizations.slice(0, 3).map((customization, index) => (
                <div key={customization._id} className="p-3 bg-gray-50 rounded-lg">
                  <div className="font-medium mb-1">{customization.name}</div>
                  <div className="text-sm text-gray-600">
                    <strong>Options:</strong> {customization.options.join(', ')}
                  </div>
                  {customization.additionalPrice > 0 && (
                    <div className="text-sm text-gray-600">
                      <strong>Additional Price:</strong> +<ProductPrice price={customization.additionalPrice} showCurrency={false} />
                    </div>
                  )}
                </div>
              ))}
