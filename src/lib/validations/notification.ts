...
export const createNotificationSchema = notificationBaseSchema.extend({
  scheduleFor: z.string()
    .refine((date) => !date || !isNaN(Date.parse(date)), 'Invalid schedule date')
    .optional(),
  expiresAt: z.string()
    .refine((date) => !date || !isNaN(Date.parse(date)), 'Invalid expiration date')
    .optional(),
}).refine((data) => {
...
export const bulkDeleteNotificationsSchema = z.object({
  notificationIds: z.array(z.string().min(1, 'Invalid notification ID'))
...
export const testNotificationSchema = z.object({
  ...
  template: z.string().min(1, 'Template is required').max(100, 'Template name is too long'),
});
...
export const scheduledNotificationSchema = createNotificationSchema.extend({
  scheduleFor: z.string()
    ...
    .refine((date) => new Date(date) > new Date(), 'Schedule date must be in the future'),
  recurring: z.object({
...
export const notificationAnalyticsSchema = z.object({
  ...
  groupBy: z.enum(['type', 'priority', 'status', 'day', 'week', 'month']).default('day'),
...
export type BulkDeleteNotificationsData = z.infer<typeof bulkDeleteNotificationsSchema>;
...
export type ScheduledNotificationData = z.infer<typeof scheduledNotificationSchema>;
export type NotificationAnalyticsData = z.infer<typeof notificationAnalyticsSchema>;
...
export const createDefaultNotificationFilterData = (): NotificationFilterData => ({
  type: 'all',
  priority: 'all',
  sortBy: 'createdAt',
  sortOrder: 'desc',
...
export const createDefaultNotificationSettings = (): NotificationSettingsData => ({
  emailNotifications: {
    newOrders: true,
...
// Validation helpers
export const validateNotificationMetadata = (type: string, metadata: any): boolean => {
...
// Schedule validation
export const validateNotificationSchedule = (scheduleFor: string, expiresAt?: string): boolean => {
