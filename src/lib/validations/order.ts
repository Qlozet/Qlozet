...
export const orderItemSchema = z.object({
  ...
  customizations: z.array(z.object({
    name: z.string().min(1, 'Customization name is required').max(100, 'Name is too long'),
    value: z.string().min(1, 'Customization value is required').max(200, 'Value is too long'),
...
export const shippingAddressSchema = addressSchema.extend({
  name: z.string().min(1, 'Recipient name is required').max(100, 'Name is too long'),
  phone: z.string()
...
const orderBaseSchema = z.object({
  orderNumber: z.string().min(1, 'Order number is required').max(50, 'Order number is too long'),
  customerId: z.string().min(1, 'Customer ID is required'),
  customerName: z.string().min(1, 'Customer name is required').max(100, 'Name is too long'),
  customerEmail: z.string().email('Invalid email format').max(255, 'Email is too long'),
  ...
  paymentMethod: z.string().min(1, 'Payment method is required').max(50, 'Payment method is too long'),
...
export const createOrderSchema = orderBaseSchema.omit({
  ...
  items: z.array(orderItemSchema.omit({ productName: true, total: true })).min(1, 'At least one item is required'),
});
...
export const orderFilterSchema = z.object({
  ...
  sortBy: z.enum(['orderNumber', 'customerName', 'total', 'createdAt', 'status']).default('createdAt'),
...
export const orderBulkUpdateSchema = z.object({
  orderIds: z.array(z.string().min(1, 'Invalid order ID')).min(1, 'At least one order must be selected'),
  updates: z.object({
    status: orderStatusSchema.optional(),
    paymentStatus: paymentStatusSchema.optional(),
...
// Validation helpers
export const calculateOrderTotal = (items: OrderItemData[], tax = 0, shipping = 0, discount = 0): number => {
