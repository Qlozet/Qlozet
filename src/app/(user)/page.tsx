...
import { DashboardTemplate } from '@/pattern/dashboard/templates/dashboard-template';
import {
...
const Dashboard: React.FC = () => {
  ...
  if (
    locationsData?.data?.data?.locations &&
    locationsData?.data?.data?.totalOrders
  ) {
  ...
  if (
    productsData?.data?.data?.topProducts &&
    productsData?.data?.data?.totalOrders
  ) {
    ...
    metrics: [
      {
        ...
        value: totalOrders,
        icon: (
          <img src={TotalOrderIcon} alt='Total Orders' className='h-6 w-6' />
        ),
        bgColor: 'bg-[#57CAEB]',
        link: 'orders',
      ...
      },
      {
        ...
        icon: (
          <img src={customerIcon} alt='Total Customers' className='h-6 w-6' />
        ),
        bgColor: 'bg-[#FF7976]',
        trend: { value: 2.5, isPositive: true },
      },
  ...
  return (
    <div className='w-full min-h-screen h-fit'>
      <DashboardTemplate data={templateData} isLoading={false} />
    </div>
  )
};
