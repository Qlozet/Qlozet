...
export const ProductCustomizationSection: React.FC<
...
> = ({ form }) => {
  ...
  const addCustomization = () => {
    ...
    form.setValue('customizations', [
      ...current,
      { name: '', options: [''], additionalPrice: 0 },
    ]);
  };
  ...
  const updateCustomization = (
    index: number,
    field: keyof ProductCustomizationData,
    value: any
  ) => {
    const current = form.getValues('customizations') || [];
  ...
  const updateOption = (custIndex: number, optIndex: number, value: string) => {
    ...
    const updated = current.map((item, i) =>
      i === custIndex
        ? {
            ...item,
            options: item.options.map((opt: string, j: number) =>
                      ...
                      (option: string, optIndex: number) => (
                        <div key={optIndex} className='flex gap-2'>
                          <Input
                            ...
                            value={option || ''}
                            onChange={(e) =>
                              updateOption(custIndex, optIndex, e.target.value)
                            }
                            className='flex-1'
                          />
