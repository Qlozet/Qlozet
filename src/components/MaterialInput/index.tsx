...
interface MaterialInputProps {
  ...
  placeholder?: string;
  disabled?: boolean;
  removeMaterialHandler: (item: string, index: number) => void;
  loading?: boolean;
}
...
const MaterialInput: React.FC<MaterialInputProps> = ({
  ...
  const handleUploads = (e: React.ChangeEvent<HTMLInputElement>): void => {
  ...
  return (
    <div>
      ...
      <div className='border-[1.5px] border-solid border-gray-200 relative h-[3rem] rounded-[8px]'>
        ...
        <div className='scrollbar-hide flex items-center gap-4 h-full pl-2'>
          {items.map((item: string, index: number) => {
            ...
            if (item) {
              return (
                ...
                >
                  <div className='absolute top-[-5px] right-[-5px] bg-primary-100 rounded-[50%] p-[1px]'>
                    <Image
                      alt='close'
                      src={closeIcon}
                      width={15}
