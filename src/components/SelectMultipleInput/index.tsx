...
import { filterExcludedItems } from '@/lib/utils';
...
const SelectMultipleInput = ({
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`flex items-center gap-2 w-full border-solid border-[1.5px]  rounded-[8px] text-dark  ${showDropDown ? 'bg-[#DDE2E5] border-primary-100' : 'border-gray-2'}`}
        >
          <div
            className={`w-full flex items-center gap-4 text-sm text-font-light placeholder:font-300 px-2`}
            onClick={() => {
              setShowDropDown(!showDropDown);
            }}
          >
            <div className='scrollbar-hide overflow-x-auto flex items-center gap-4 w-[95%] py-2'>
              {selectedList.length > 0 ? (
                selectedList.map((item) => (
                  <span
                    className='flex items-center gap-1 p-1 bg-gray-700 rounded-[6px] text-sm'
                    onClick={() => {
                      removeItem(item.id);
                    }}
                    key={item.id}
                  >
                    {item.text}{' '}
                    <button>
                      <X width={16} height={16} color={'#777777'} />
                    </button>
                  </span>
                ))
              ) : (
            ...
            <button className='absolute top-[40px] right-[16px]'>
      ...
      {showDropDown && (
        <div className='cursor-pointer absolute top-[75px] bg-white rounded-lg max-h-[15rem] datalist-scroll shadow-md w-full'>
          {list.length > 0 ? (
            <>
              {list.map((item, index) => (
                <div
