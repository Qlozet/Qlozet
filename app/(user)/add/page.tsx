...
import SingleCustomzeCard from "@/components/Products/SingleCustomizeCard";
import { useFormik } from "formik";
import { object, string, number, date, InferType, array } from "yup";
import RichTextEditor from "@/components/Editor";
import { AddProductFormData, ProductImageUpload, StyleVariantUpload, AccessoryItem, VariantTableItem, PositionStyle } from "@/types";
const AddProduct: React.FC = () => {
  ...
  const [variantTable, setVariantTable] = useState<VariantTableItem[]>([]);
  ...
  const [positionStyles, setPositionStyles] = useState<PositionStyle[]>([]);
  const [accessories, setAcessories] = useState<AccessoryItem[]>([]);
  ...
  // Functions
  const handleSelectFile = async (files: File[], deletedFiles?: File[]) => {
  ...
  const submitAcessories = (acess: { images: string; id: string; price: number }) => {
  ...
  const handleSelectStyle = (positionStyles: PositionStyle[], width: number, height: number) => {
  ...
  const submitVariantImage = async (file: File, id: string, imageIndex: number, color: string) => {
  ...
  const addToVariantTable = async (data: File | any) => {
                  ...
                  <div className="flex flex-col gap-6">
                    <SingleCustomzeCard
                      ...
                      children={
                        <div>
                          <div
                            className="flex overflow-x-scroll gap-4 pb-2 custom-product-scrollbar"
                          >
                            <CustomiSationButton
                              handleClick={() => {
                    ...
                    <SingleCustomzeCard
                      ...
                      children={
                        <div
                          className="flex overflow-x-scroll gap-4 custom-product-scrollbar"
                        >
                          <CustomiSationButton
                            handleClick={() => {
