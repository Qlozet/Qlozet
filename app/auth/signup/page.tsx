...
const SignUp: React.FC = () => {
  ...
  const [businessLogo, setBusinessLogo] = useState<File[]>([]);
  ...
  const [requiredFormData, setrequiredFormData] = useState<SignupFormValidation>({
    businessName: false,
    businessEmail: false,
  ...
  const handleBusinessInfo = () => {
    const { status, data } = validator(businessInfo, requiredFormData) as ValidatorResult;
    if (status) {
      setStep(step + 1);
    } else {
  ...
  const handlePersonalInfo = () => {
    const { status, data } = validator(personalInfo, requiredFormData) as ValidatorResult;
    if (status) {
      setStep(step + 1);
    } else {
      setrequiredFormData((prevData) => {
  ...
  const handleSubmit = async () => {
    const { status, data } = validator(passwordInfo, requiredFormData) as ValidatorResult;
    try {
      if (status) {
        setIsLoading(true);
        ...
        businessFiles.forEach((image) => {
          formData.append("cacDocument", image);
        });
        if (businessLogo[0]) {
          formData.append("businessLogo", businessLogo[0]);
        }
        const response = await postRequest(`/vendor/signup`, formData, true) as ApiResponse;
